...
def load_ecg(file_path: str, sampling_rate: int = 1000) -> np.ndarray:
    ...
    ecg_channel = [ch for ch in raw.ch_names if 'ECG' in ch.upper() or 'EKG' in ch.upper()][0]
    return raw.get_data(picks=ecg_channel)[0]

def process_file(file_path: str, intervals: Dict[str, List[Tuple[float, float]]], sampling_rate: int = 1000) -> Dict[str, List[np.ndarray]]:
    ...
    for state, segments in intervals.items():
        for start_time, end_time in segments:
            ...
            sub_segments = processor.segment_signal(segment_signal, overlap=0.9)
            
            for sub_segment in sub_segments:
                r_peaks = nk.ecg_findpeaks(sub_segment, sampling_rate=sampling_rate)
                rr_intervals = np.diff(r_peaks['ECG_R_Peaks']) / sampling_rate
                
                if len(rr_intervals) > 0:
                    features = processor.calculate_features(sub_segment, rr_intervals)
